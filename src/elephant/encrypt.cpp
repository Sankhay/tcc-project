...
void useElephant(BYTE key[CRYPTO_ABYTES], BYTE nonce[CRYPTO_NPUBBYTES], BYTE* plaintext, BYTE* ad, AlgorithmReturn* algorithmReturn) {
    ...
    static BYTE ciphertext[PLAINTEXT_SIZE + CRYPTO_ABYTES];
    static unsigned long long ciphertext_len = 0;
    
    int success = crypto_aead_encrypt(ciphertext, &ciphertext_len,
    ...
    algorithmReturn->encryptedData = malloc(sizeof(BYTE) * (PLAINTEXT_SIZE + CRYPTO_ABYTES));
    memcpy(algorithmReturn->encryptedData, ciphertext, sizeof(BYTE) * (PLAINTEXT_SIZE + CRYPTO_ABYTES));

    // Decryption
    unsigned char decrypted[PLAINTEXT_SIZE + CRYPTO_ABYTES];
    unsigned long long decrypted_len = 0;
    
    int success_decrypt = crypto_aead_decrypt(decrypted, &decrypted_len,
                           NULL,
                           ciphertext, ciphertext_len,
                           ad, 0,
                           nonce, key);
    
    if (success == 0 && success_decrypt == 0) {
